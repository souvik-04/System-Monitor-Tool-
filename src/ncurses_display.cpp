...
void NCursesDisplay::DisplaySystem(System& system, WINDOW* window) {
  ...
  wattroff(window, COLOR_PAIR(1));
  mvwprintw(window, ++row, 2, "%s", ("Total Processes: " + to_string(system.TotalProcesses())).c_str());
  mvwprintw(window, ++row, 2, "%s", ("Running Processes: " + to_string(system.RunningProcesses())).c_str());
  mvwprintw(window, ++row, 2, "%s", ("Up Time: " + Format::ElapsedTime(system.UpTime())).c_str());
  wrefresh(window);
}
...
void NCursesDisplay::DisplayProcesses(std::vector<Process> processes,
  ...
  // Safety: do not attempt to print more processes than available
  int to_print = std::min(static_cast<int>(processes.size()), n);

  int win_x = getmaxx(window);
  ...
  for (int i = 0; i < to_print; ++i) {
    ...
    // ensure we don't request a negative or too-large substring
    int max_cmd_len = std::max(0, win_x - command_column - 2);
    string cmd = processes[i].Command();
    if (static_cast<int>(cmd.size()) > max_cmd_len) {
      if (max_cmd_len > 3)
